...
__input__ = {"kernel": {"max_pool_grad": "max_pool_grad_input"}}
...
def max_pool_grad_input(
    ...
    input_data, _, grad_data = x1, x2, grad
    input_shape = kwargs["input_ori_shapes"][0]
    dtype = kwargs["input_dtypes"][0]

    input_data_4d = nc1hwc0_2_nhwc(input_data, input_shape)
    input = tf.compat.v1.placeholder(shape=input_shape, dtype=dtype)
    input_orig_y = tf.nn.max_pool(input, ksize, strides, padding, name="maxpool")
    ...
    return [input_data, input_orig_y_data_5d, grad_data]
