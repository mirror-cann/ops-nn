...
static const std::initializer_list<op::DataType> TARGET_DTYPE_SUPPORT_LIST = {DataType::DT_INT64, DataType::DT_INT32};
...
static bool CheckShape(const aclTensor* self, const aclTensor* target, const aclTensor* out, const aclTensor* isTarget,
                       int64_t reduction)
{
    ...
    if (selfDimNum <= 1) {
        ...
        OP_CHECK(
            ...
            OP_LOGE(ACLNN_ERR_PARAM_INVALID, "inconsistent target size: %s for self of size: %s",
                    op::ToString(target->GetViewShape()).GetString(), op::ToString(self->GetViewShape()).GetString()),
            return false);
    ...
    if (reduction == 0 && selfDimNum == MAX_SELF_DIM_NUM) {
        OP_CHECK(
            ...
            OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Expect out shape [%zu], but got: %s.", self->GetViewShape().GetDim(0),
    ...
    if (selfDimNum == MAX_SELF_DIM_NUM && self->GetViewShape().GetDim(0) == 0) {
        OP_CHECK(
            ...
            OP_LOGE(ACLNN_ERR_PARAM_INVALID, "inconsistent isTarget size: %s for self of size: %s",
                    op::ToString(isTarget->GetViewShape()).GetString(), op::ToString(self->GetViewShape()).GetString()),
            return false);
    } else {
                                                      ...
                                                      aclOpExecutor** executor)
{
    ...
    auto targetReshape = target->GetViewShape().GetDimNum() == 0 ?
                             l0op::UnsqueezeNd(targetCasted, squeezeDim, uniqueExecutor.get()) :
                             targetCasted;
    ...
    auto lossOut = l0op::MultilabelMarginLoss(selfReshape, targetReshape, GetReductionStr(reduction),
                                              out->GetViewShape(), uniqueExecutor.get());
...
aclnnStatus aclnnMultilabelMarginLoss(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
                                      aclrtStream stream)
{
