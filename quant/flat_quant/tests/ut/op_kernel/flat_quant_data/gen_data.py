...
def gen_data_and_golden(
    x_shape_str,
    kronecker_p1_shape_str,
    kronecker_p2_shape_str,
    group_list_shape_str,
    d_type="float16",
):
    d_type_dict = {
        "float16": np.float16,
    ...
    tem_kronecker_p1 = (
        np.random.random(np.prod(kronecker_p1_shape))
        .reshape(kronecker_p1_shape)
        .astype(np_type)
    )
    tem_kronecker_p2 = (
        np.random.random(np.prod(kronecker_p2_shape))
        .reshape(kronecker_p2_shape)
        .astype(np_type)
    )
    tem_group_list = np.array([1], dtype=np.int64)
    ...
    group_list = (
        None
        if group_list_shape_str == "(0)"
        else torch.tensor(tem_group_list, dtype=torch.int64)
    )

    quant_scale = do_golden(
        x_tensor.to(torch.float),
        kronecker_p1_tensor.to(torch.float),
        kronecker_p2_tensor.to(torch.float),
    )
    ...
    if group_list is not None:
        tem_group_list.tofile(f"{d_type}_input_group_list_flat_quant.bin")
