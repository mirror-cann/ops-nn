...
static inline bool CheckPlatformValid()
{
    ...
    if (curArch != NpuArch::DAV_2201) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Current npuArch [%u] is not supported.", static_cast<uint32_t>(curArch));
        return false;
    }
    return true;
}
...
static inline bool CheckDtypeValid(const aclTensor* gradOutput, const aclTensor* self,
                                   [[maybe_unused]] const aclScalar* lambd, const aclTensor* gradInput)
{
    // 检查gradOutput的数据类型是否在SoftshrinkGrad算子的支持列表内
    OP_CHECK_DTYPE_NOT_SUPPORT(gradOutput, ASCEND910B_DTYPE_SUPPORT_LIST, return false);

    // 检查self的数据类型是否在SoftshrinkGrad算子的支持列表内
    OP_CHECK_DTYPE_NOT_SUPPORT(self, ASCEND910B_DTYPE_SUPPORT_LIST, return false);

    // 检查gradInput的数据类型是否在支持列表内
    OP_CHECK_DTYPE_NOT_SUPPORT(gradInput, ASCEND910B_DTYPE_SUPPORT_LIST, return false);
                                                    ...
                                                    uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    // 将计算结果转换为gradInput的数据类型
    auto castOut = softShrinkBackwardOpOut;
    if (softShrinkBackwardOpOut->GetDataType() != gradInput->GetDataType()) {
        castOut = l0op::Cast(softShrinkBackwardOpOut, gradInput->GetDataType(), uniqueExecutor.get());
