...
static const std::initializer_list<op::DataType> DTYPE_310B_SUPPORT_ATOMIC = {
    op::DataType::DT_FLOAT, op::DataType::DT_INT32, op::DataType::DT_FLOAT16, op::DataType::DT_BOOL,
    op::DataType::DT_BF16};
...
static bool CheckNotNull(const aclTensor* self, const aclTensorList* indices, const aclTensor* values)
{
                                               ...
                                               const aclTensor* value)
{
    ...
    for (int i = 0; i < static_cast<int>(indices.size()); i++) {
        if (indices[i]->GetDataType() != op::DataType::DT_INT32 &&
            indices[i]->GetDataType() != op::DataType::DT_INT64) {
            return true;
        }
    }
...
static bool IsAiCPUSupportCheckIndices(const FVector<const aclTensor*, 8>& indices, const aclTensor* value)
{
    /**
     * Using AICPU when allDefinedIndices size is more than two or the type of selfRef tensor is double.
     * Otherwise AICORE, indices tensors are at the discontinuous axis position, implemented by AICPU.
                           ...
                           const aclTensor* value, const bool accumulate, const FVector<int64_t, 8> masks)
{
    ...
    if (indices[0]->GetViewShape().GetShapeSize() > MAX_RESERVE_NUM || tailSize > MULTI_DTYPE_SUPPORT_TAIL ||
        ...
        if (is310BSocVersion && !CheckType(selfRef->GetDataType(), DTYPE_310B_SUPPORT_ATOMIC)) {
            OP_LOGD(
                "IndexPutV2 Indices_number > 100 and input_dtype is uint8, int8 or int64, aicore does not support.");
            return true;
        }
    }
    return false;
...
static const aclTensor* AdaptValueforBroadcast(int64_t valueDimNum, op::Shape& oriValueShape,
...
static const aclTensor* AicoreCompute(const aclTensor* selfCast, const FVector<const aclTensor*, 8>& allDefinedIndices,
                                      ...
                                      const aclTensor* maskTensor, const FVector<int64_t, 8> masks,
                                      const bool accumulate, int64_t masksNum, int64_t indicesNum,
                                      aclOpExecutor* executor)
{
    ...
    if (masksNum != indicesNum && tailSize < TAIL_SIZE) {
        ...
        indexPutOpOut = l0op::IndexPutV2(selfCast, indicesTensorList, valueBroadcast, masktensor_trans, accumulate, out,
                                         executor);
        ...
        aclTensor* out = const_cast<aclTensor*>(selfCast);
        indexPutOpOut = l0op::IndexPutV2(selfCast, indicesTensorList, valueBroadcast, maskTensor, accumulate, out,
                                         executor);
    }
...
static const aclTensor* valuesToBroadcast(int64_t indicesSize, const aclTensor* selfCast, const aclTensorList* indices,
                                          FVector<const aclTensor*> definedIndices, const aclTensor* valuesCast,
                                          aclOpExecutor* executor)
{
...
static const aclTensor* valuesToBroadcastArch3510(const aclTensor* selfCast, FVector<const aclTensor*> definedIndices,
                                                  ...
                                                  bool iscontiguousIdx, aclOpExecutor* executor)
{
    ...
    if (iscontiguousIdx) {
        if (masks[0] == 0) { // some case like: indices mask=[0110]
            auto selfIndex = 0;
            auto updateIndex = 0;
            while (updateIndex < valueDimNum && selfIndex < selfDimNum && masks[selfIndex] == 0) { // add former x_shape
...
static bool isBroadCastShape(FVector<const aclTensor*, DIMLIMIT>& allIndices, int i, std::vector<int64_t> tensorShape,
                             uint64_t tensorShapeDim)
{
    bool needBroadcast = false;
                                          ...
                                          aclOpExecutor* executor)
{
    ...
    const aclTensor* indexPutOpOut;
    aclTensor* out = const_cast<aclTensor*>(selfCast);
    indexPutOpOut = l0op::IndexPutV2(selfCast, allIndicesTensorList, valueBroadcast, maskTensor, accumulate, out,
                                     executor);
...
static const aclTensor* SortedIndexPutProcess(const aclTensor* selfCast, const aclTensor* valuesCast,
                                              const aclTensorList* indices, const bool accumulate,
                                              const FVector<int64_t, 8> masks, int64_t masksNum,
                                              aclOpExecutor* executor)
{
    ...
    // Value Broadcast
    auto valueBroadcast = valuesToBroadcastArch3510(selfCast, definedIndices, valuesCast, masks, iscontiguousIdx,
                                                    executor);
    CHECK_RET(valueBroadcast != nullptr, nullptr);
    ...
    indexPutOpOut = l0op::IndexPutWithSortV2(selfCast, sortIdxInt, posIdx, valueBroadcast, maskArray, accumulate, out,
                                             executor);
    CHECK_RET(indexPutOpOut != nullptr, nullptr);
    return indexPutOpOut;
}

static const aclTensor* IndexPutProcessArch3510(aclTensor* selfRef, const aclTensor* selfCast,
                                                const aclTensor* valuesCast, const aclTensorList* indices,
                                                const FVector<const aclTensor*, 8>& allDefinedIndices,
                                                const bool accumulate, FVector<int64_t, 8> masks, int64_t masksNum,
                                                const bool isSupportAiCpu, const bool isNonContiguous,
                                                const bool useSortedV2Opt, aclOpExecutor* executor)
{
    ...
    if (isSupportAiCpu) {
        ...
        indexPutOpOut = AicpuProcess(selfRef, selfCast, valuesCast, allDefinedIndices, accumulate, masks, masksNum,
                                     executor);
    } else {
...
static bool CheckIsContiguous(bool selfContiguous, bool valueContiguous, bool indicesContiguous, bool overLimit,
                              bool isIndicesTranspose, bool isSelfTranspose, bool isValueTranspose,
                              bool isIndicesStridesAllZero, bool isSelfStridesAllZero)
{
...
static bool IsNonContiguousScene(const aclTensor* self, const aclTensorList* indices, const aclTensor* value,
                                 ...
                                 FVector<int64_t, DIMLIMIT>& masks, int64_t& indicesNum, int64_t& masksNum,
                                 const aclTensor*& selfRefContiguous, const aclTensor*& valuesContiguous,
                                 aclOpExecutor* executor)
{
    ...
    if (selfContiguous == false) {
        selfRefContiguous = executor->CreateView(self, self->GetViewShape(), self->GetStorageShape(),
                                                 self->GetViewStrides(), self->GetViewOffset());
    }
    if (valueContiguous == false) {
        valuesContiguous = executor->CreateView(value, value->GetViewShape(), value->GetStorageShape(),
...
bool IsSpecialScene(const aclTensor* selfRef, FVector<int64_t, DIMLIMIT> masks, const bool& accumulate)
{
    ...
    std::set<op::DataType> supportType = {op::DataType::DT_BOOL,    op::DataType::DT_INT8, op::DataType::DT_UINT8,
                                          op::DataType::DT_FLOAT16, op::DataType::DT_BF16, op::DataType::DT_INT32,
...
aclnnStatus aclnnIndexPutImplGetWorkspaceSize(aclTensor* selfRef, const aclTensorList* indices, const aclTensor* values,
                                              const bool accumulate, const bool unsafe, uint64_t* workspaceSize,
                                              aclOpExecutor** executor)
{
    ...
    if (GetCurrentPlatformInfo().GetSocVersion() == SocVersion::ASCEND910B ||
        ...
        if (indicesSize <= static_cast<int64_t>(MAX_SUPPORT_DIMS_NUMS)) {
            ...
            for (size_t i = 0; i < dimMask.size(); i++) {
                if (dimMask[i]) {
                    int64_t bound = (selfRef->GetViewShape().GetDimNum() == 0) ? 1 : selfRef->GetViewShape().GetDim(i);
    ...
    bool nonContiguous = IsNonContiguousScene(selfRef, indices, values, allDefinedIndices, masks, indicesNum, masksNum,
                                              selfRefContiguous, valuesContiguous,
                                              uniqueExecutor.get()); // 是否满足非连续场景条件
    ...
    bool isHighPrecision = accumulate && (selfRef->GetDataType() == op::DataType::DT_FLOAT16 ||
                                          selfRef->GetDataType() == op::DataType::DT_BF16 ||
                                          selfRef->GetDataType() == op::DataType::DT_INT8 ||
                                          selfRef->GetDataType() == op::DataType::DT_UINT8);
    bool isNonContiguous = isIndexPutV2 && nonContiguous && !isAiCpu &&
                           !isHighPrecision; // 非连续+AiCore+IndexPutV2分支
    ...
    if (Ops::NN::AclnnUtil::IsRegbase()) {
        ...
        const bool useSortedV2Opt = l0op::IsUseSortedV2OptScene(isAiCpu, selfRef, indices, values, deterministicValue,
                                                                accumulate, isNonContiguous, usePutV2SpeOpt);
        const bool disDeterministicHighPrecision = accumulate && deterministicValue == 0 && !useSortedV2Opt &&
                                                   (selfRef->GetDataType() == op::DataType::DT_FLOAT16 ||
                                                    selfRef->GetDataType() == op::DataType::DT_BF16 ||
                                                    selfRef->GetDataType() == op::DataType::DT_INT8 ||
                                                    selfRef->GetDataType() == op::DataType::DT_UINT8);
        if (disDeterministicHighPrecision) {
            ...
            if ((selfRef->GetDataType() == op::DataType::DT_INT8 && !usePutV2SpeOpt) ||
                selfRef->GetDataType() == op::DataType::DT_UINT8) {
        ...
        indexPutOpOut = IndexPutProcessArch3510(selfRef, selfCast, valuesCast, indices, allDefinedIndices, accumulate,
                                                masks, masksNum, isAiCpu, isNonContiguous, useSortedV2Opt,
                                                uniqueExecutor.get());
    ...
    bool useIndexPutWithSortSupport = l0op::IsIndexPutWithSortSupport(selfRef, indices, values, deterministicValue,
                                                                      accumulate);
    if (!useIndexPutWithSortSupport) {
        ...
        if (isSupportAiCpu) {
            aclTensor* out = const_cast<aclTensor*>(selfCast);
            if (deterministicValue != 0) {
                indexPutOpOut = l0op::IndexPutV3(selfCast, indicesTensorList, valuesCast, maskTensor, accumulate, true,
                                                 out, uniqueExecutor.get());
            } else {
                indexPutOpOut = l0op::IndexPut(selfCast, indicesTensorList, valuesCast, maskTensor, accumulate, out,
                                               uniqueExecutor.get());
            }
        } else {
            indexPutOpOut = AicoreCompute(selfCast, allDefinedIndices, indicesTensorList, valuesCast, maskTensor, masks,
                                          accumulate, masksNum, indicesNum, uniqueExecutor.get());
        }
        CHECK_RET(indexPutOpOut != nullptr, ACLNN_ERR_INNER_NULLPTR);
    } else {
        ...
        // values做broadcast操作
        auto valueBroadcast = valuesToBroadcast(indicesSize, selfCast, indices, definedIndices, valuesCast,
                                                uniqueExecutor.get());

        auto strideTensor = uniqueExecutor.get()->ConvertToTensor(strideTail.data(), strideTail.size(),
                                                                  DataType::DT_INT32);
        auto valueSizeTensor = uniqueExecutor.get()->ConvertToTensor(valueSizeTail.data(), valueSizeTail.size(),
                                                                     DataType::DT_INT32);
        ...
        aclTensor* out = const_cast<aclTensor*>(selfCast);
        indexPutOpOut = l0op::IndexPutWithSort(selfCast, sortIdxInt, posIdx, valueBroadcast, sliceSize, accumulate, out,
                                               uniqueExecutor.get());
        CHECK_RET(indexPutOpOut != nullptr, ACLNN_ERR_INNER_NULLPTR);
    }
