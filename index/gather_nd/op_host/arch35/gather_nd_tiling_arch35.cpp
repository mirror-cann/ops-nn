...
bool GatherNdSimtTiling::IsCapable() { return true; }
...
ge::graphStatus GatherNdSimtTiling::GetIndicesShapeInfo()
{
    ...
    if (curIndicesDimLen == static_cast<uint64_t>(0)) {
        OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(context_->GetNodeName(), "indices", "0D",
    ...
    for (uint64_t i = static_cast<uint64_t>(0); i < curIndicesDimLen - static_cast<uint64_t>(1); i++) {
        OP_LOGD("GatherNdSimt", "IndicesShape: indices axis is %ld, shape value is %ld", i,
                orgIndicesGeShape.GetDim(i));
...
void GatherNdSimtTiling::CalcSimdTiling()
{
    ...
    uint64_t ubAviable = static_cast<uint64_t>((ubSize_ - INDICES_SIZE) / ubBlockSize * ubBlockSize / xDtypeSize_ /
                                               BUFFER_NUM);
...
ge::graphStatus GatherNdSimtTiling::GetXShapeInfo()
{
    ...
    } else if ((xDtype == ge::DT_FLOAT16) || (xDtype == ge::DT_BF16) || (xDtype == ge::DT_INT16) ||
               (xDtype == ge::DT_UINT16)) {
    ...
    if (curXDimLen == 0) {
        OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(context_->GetNodeName(), "x", "0D",
...
ge::graphStatus GatherNdSimtTiling::GetShapeAttrsInfo()
{
    ...
    for (int64_t i = static_cast<int64_t>(rank_ + static_cast<uint64_t>(1));
         i < static_cast<int64_t>(GATHER_ND_X_DIM_LEN); i++) {
    ...
    if ((!isZeroShape_) && (rank_ > static_cast<uint64_t>(0)) && !negativeIndexSupport_ && XDtypeImprove()) {
        OP_LOGD("GatherNdSimt", "XDtypeImprove before gatherSize_ %ld, xDtypeSize_ is %ld", gatherSize_, xDtypeSize_);
        gatherSize_ = gatherSize_ / (improveDtypeSize_ / xDtypeSize_);
        xShape_[rank_] = gatherSize_;
        outputSize_ = indicesNum_ * gatherSize_;
        xDtypeSize_ = improveDtypeSize_;
        OP_LOGD("GatherNdSimt", "XDtypeImprove after gatherSize_ %ld, xDtypeSize_ is %ld", gatherSize_, xDtypeSize_);
    }
...
bool GatherNdSimtTiling::IsGaAllLoad() const
{
    ...
    if ((gatherSize_ <= WARP_THREAD_NUM || gatherSize_ >= SIMD_THRES) &&
        indicesNum_ / coreNum_ / gatherDimSize_ < static_cast<uint32_t>(NUM_TWO)) {
        return false;
    }
    ...
    int64_t remainSpace = ubSize - static_cast<int64_t>(HELP_BUFFER_SIZE) -
                          NUM_TWO * static_cast<int64_t>(MIN_OUT_UB_SIZE) -
...
ge::graphStatus GatherNdSimtTiling::GaAllLoadTiling()
{
    ...
    indicesBufferSizeLoopGa = std::min(indicesInner * ubBlockSize,
                                       static_cast<int64_t>(ubSize_ - NUM_TWO * MIN_OUT_UB_SIZE - allLoadXBytes));
...
ge::graphStatus GatherNdSimtTiling::DoOpTiling()
{
    ...
    if (gatherSize_ * xDtypeSize_ >= SIMD_THRES && ubSize_ >= BUFFER_NUM * INDICES_SIZE &&
        indicesNum_ >= coreNum_ / BUFFER_NUM) {
        isSimd_ = true;
        CalcSimdTiling();
        return ge::GRAPH_SUCCESS;
    }
...
ge::graphStatus GatherNdSimtTiling::DoBroadCastOpTiling()
{
    ...
    uint64_t ubCopySize = ubSize_ / static_cast<uint64_t>(2) / static_cast<uint64_t>(UB_SIZE_ALIGN) *
                          static_cast<uint64_t>(UB_SIZE_ALIGN);
    xUbSize_ = ubCopySize;
...
ge::graphStatus GatherNdSimtTiling::DoNormOpTiling()
{
    ...
    uint64_t gatherCount = (static_cast<uint64_t>(ubSize_) - static_cast<uint64_t>(INDICES_REDUENT_SIZE)) /
                           (xRatio + indicesRatio);
    ...
    OP_LOGD("GatherNdSimt", "xUbSize_ * coreNum_ is %ld, outputSize is %ld", xUbSize_ * coreNum_, outputSize_);
...
ge::graphStatus GatherNdSimtTiling::DoMixKernelOpTiling()
{
    ...
    uint64_t availableSingleBuf = (ubSize_ - static_cast<uint64_t>(HELP_BUFFER_SIZE + RESERVE_BUFFER_SIZE)) /
                                  BUFFER_NUM;
    ...
    uint64_t ubBlockSize = static_cast<uint64_t>(GetUbBlockSize(context_));
    uint64_t yUbSize = (singleUbProNum * xLastAxisBytes + ubBlockSize - static_cast<uint64_t>(1)) / ubBlockSize *
                       ubBlockSize;
    uint64_t indicesUbSize = (singleUbProNum * singleIndicesBytes + ubBlockSize - static_cast<uint64_t>(1)) /
...
ge::graphStatus GatherNdSimtTiling::DoLibApiTiling() { return ge::GRAPH_SUCCESS; }
...
uint64_t GatherNdSimtTiling::GetTilingKey() const
{
    if (isSimd_) {
        ...
        uint64_t tilingBase = negativeIndexSupport_ ? static_cast<uint64_t>(NEG_INDICES_SUPPORT_BASE_KEY) :
    ...
    if (isMixKernel_) {
        ...
        uint64_t units = (indicesNum_ > static_cast<uint64_t>(INT32_MAX) ||
                          outputSize_ > static_cast<uint64_t>(INT32_MAX)) ?
    ...
    uint32_t indicesKey = (indicesDtypeSize_ == INPUT_DTYPE_B64 || rank_ == static_cast<uint32_t>(0) || isZeroShape_) ?
                              static_cast<uint32_t>(DTYPE_B64_KEY) :
                              static_cast<uint32_t>(DTYPE_B32_KEY);
    ...
    uint32_t supportAbnormalIndexKey = (negativeIndexSupport_ || supportOutOfBoundIndex_) ? static_cast<uint32_t>(1) :
                                                                                            static_cast<uint32_t>(0);
    uint32_t sizeAddrType = ((gatherSize_ > static_cast<uint64_t>(INT32_MAX) &&
                              indicesNum_ > static_cast<uint64_t>(INT32_MAX)) ||
                             outputSize_ > static_cast<uint64_t>(INT32_MAX)) ?
                                static_cast<uint32_t>(1) :
                                static_cast<uint32_t>(0);
    uint32_t templateKey = (rank_ == static_cast<uint64_t>(0) || isZeroShape_) ? static_cast<uint32_t>(1) :
                                                                                 static_cast<uint32_t>(0);
    uint32_t rankKey = (rank_ == static_cast<uint64_t>(0) || isZeroShape_) ? static_cast<uint32_t>(0) :
                                                                             static_cast<uint32_t>(1);
    if (rankKey == static_cast<uint32_t>(0)) {
        supportAbnormalIndexKey = static_cast<uint32_t>(0);
        indicesKey = static_cast<uint32_t>(0);
    }
    uint64_t tilingKey = Ops::NN::Optiling::GET_TILINGKEY(
        indicesKey, xKey, supportAbnormalIndexKey, sizeAddrType, templateKey, static_cast<uint32_t>(1),
        static_cast<uint32_t>(1), static_cast<uint32_t>(3), static_cast<uint32_t>(2), rankKey);
