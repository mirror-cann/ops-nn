...
/*!
 * \file group_norm_silu_quant_regbase_empty_tensor.h
 * \brief GroupNormSiluQuant arch35 空张量场景(reduce 轴为空/无 x 数据): 无归一化/silu/量化, 仅按 N*numGroups 写出
 * mean/rstd。
 */

#ifndef GROUP_NORM_SILU_QUANT_REGBASE_EMPTY_
#define GROUP_NORM_SILU_QUANT_REGBASE_EMPTY_
...
template <typename T>
class GroupNormSiluQuantEmpty {
public:
    __aicore__ inline GroupNormSiluQuantEmpty(){};
    // quantScale/silu 在空张量场景无意义, 仅为与入口统一调用签名保留(未使用)
...
private:
    const GroupNormSiluQuantRegbaseTilingData* tiling_;
    TPipe pipe_;
